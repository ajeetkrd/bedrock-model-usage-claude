...
counts (including prompt-cache tokens) using the configured per-model price table.
"""
...
def _cost_usd(
    ...
    rin, rout, rcw, rcr = _rates_for(model_id)
    return (
        Decimal(int(input_tokens)) * Decimal(str(rin))
        + Decimal(int(output_tokens)) * Decimal(str(rout))
        + Decimal(int(cache_write_tokens)) * Decimal(str(rcw))
        + Decimal(int(cache_read_tokens)) * Decimal(str(rcr))
    ) / Decimal(1_000_000)
...
def _render(items: list[dict], month: str) -> str:
    ...
    for it in items:
        ...
        cw = int(it.get("cacheWriteTokens", 0) or 0)
        cr = int(it.get("cacheReadTokens", 0) or 0)
        t = int(it.get("totalTokens", 0) or 0)
        inv = int(it.get("invocations", 0) or 0)
        cost = _cost_usd(model_id, i, o, cw, cr)
