...
def cost_usd(model_id: str, input_tokens: int, output_tokens: int) -> Decimal:
    rate_in, rate_out = _rates_for(model_id)
...
def _extract_tokens(message: dict) -> tuple[int, int]:
    ...
    if not input_tokens or not output_tokens:
        body = output_block.get("outputBodyJson") or {}
...
def _process_object(key: str) -> dict[tuple[str, str, str], dict]:
    ...
    agg: dict[tuple[str, str, str], dict] = defaultdict(
        lambda: {"input": 0, "output": 0, "count": 0}
    )
...
# ---------------------------------------------------------------------------
def _apply_delta(
    user: str, model_id: str, month: str, in_tok: int, out_tok: int, count: int
) -> None:
    pk = f"USER#{user}"
    ...
    table.update_item(
        Key={"pk": pk, "sk": sk},
        UpdateExpression=(
            "ADD totalTokens :d, inputTokens :i, outputTokens :o, invocations :c "
...
def _user_month_cost(user: str, month: str) -> Decimal:
    ...
    kwargs = {
        ...
        "FilterExpression": Attr("month").eq(month) & Attr("sk").begins_with("MODEL#"),
    }
    while True:
...
def _emit_emf(model_totals: dict[str, dict]) -> None:
    ...
    for model_id, t in model_totals.items():
        cost = cost_usd(model_id, t["input"], t["output"])
        doc = {
            "_aws": {
...
def handler(event: dict, _context: Any) -> dict:
    ...
    model_totals: dict[str, dict] = defaultdict(lambda: {"input": 0, "output": 0, "count": 0})
    ...
    for key in new_keys:
        ...
        for (user, model_id, month), cell in agg.items():
            try:
                _apply_delta(
                    user, model_id, month, cell["input"], cell["output"], cell["count"]
                )
            except ClientError:
            ...
            user_month_cost_delta[(user, month)] += cost_usd(
                model_id, cell["input"], cell["output"]
            )
