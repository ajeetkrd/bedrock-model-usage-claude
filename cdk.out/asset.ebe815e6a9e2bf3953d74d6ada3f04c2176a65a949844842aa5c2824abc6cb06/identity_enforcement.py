...
# How long to suppress repeat DRYRUN/ERROR notifications for the same user/month
# so a still-over-budget user (dry-run) or a persistent failure doesn't email on
# every run. A genuine REMOVED action always notifies (it only happens when the
# user is an active member, i.e. newly re-added). Default: once per 24h.
RENOTIFY_SECONDS = int(os.environ.get("ENFORCE_RENOTIFY_HOURS", "24")) * 3600
STATE_TABLE = os.environ.get("STATE_TABLE", "")
# SNS topic for operator alerts. The batch processor already sets this env var
# and its role already has sns:Publish on the topic, so notifying here needs no
# IAM or stack change. The topic lives in the stack/Lambda region, so the SNS
# client uses the default region (not the Identity Store region).
...
def _sns_client():
    """Lazily build an SNS client in the Lambda's own region (where the alert
    topic lives), independent of the Identity Store region."""
    global _sns
    if _sns is None:
...
def _notify(subject: str, body: str, user: str, level: str) -> None:
    """Publish an enforcement notification to the alert topic. Best-effort: a
    failure here must never break the rollup, so all errors are swallowed."""
    ...
    try:
        _sns_client().publish(
            ...
            MessageAttributes={
                ...
                "kind": {"DataType": "String", "StringValue": "enforcement"},
            },
        )
        logger.info("enforce: published %s notification for user=%s", level, user)
    except ClientError:
        logger.exception("enforce: failed to publish %s notification for user=%s", level, user)
...
def _get_marker(user: str, month: str) -> dict | None:
    """Return the prior enforcement marker for (user, month), or None."""
...
def enforce_user(user: str, month: str) -> None:
    ...
    # Reconciliation model (NOT one-shot): we re-check the user's group
    # membership every run they are over budget. The actual DeleteGroupMembership
    # only fires when they are still a member, so a user who was removed and
    # stays removed generates no new Bedrock logs and is never re-evaluated. A
    # user who was manually re-added and keeps spending IS removed again. The
    # prior marker is used only to throttle repeat DRYRUN/ERROR emails, never to
    # block a real removal.
    prior = _get_marker(user, month)
    ...
    # Outcome precedence for the audit record:
    #   REMOVED  -> a real removal happened this run (always notifies)
    #   ERROR    -> something failed (retried next run; throttled notify)
    #   DRYRUN   -> would have removed (re-evaluated next run; throttled notify)
    #   SKIPPED  -> user was in none of the groups (silent)
    if removed:
        mode = "REMOVED"
    elif errored:
        mode = "ERROR"
    ...
    _notify_outcome(user, month, mode, user_id, removed, dryrun, errored, prior)


def _fmt_groups(entries: list[dict]) -> str:
    """Render a list of group entries as 'name (groupId[, membership=...])'."""
    parts = []
    for e in entries:
        label = f"{e.get('group')} ({e.get('groupId')}"
        if e.get("membershipId"):
            label += f", membership={e['membershipId']}"
        if e.get("reason"):
            label += f", reason={e['reason']}"
        parts.append(label + ")")
...
def _notify_outcome(
    ...
    """Send an operator email describing what enforcement just did.

    SKIPPED is silent. REMOVED always notifies (a real access change just
    happened). DRYRUN/ERROR are throttled: if the prior marker for this
    user/month was the same mode and was written within RENOTIFY_SECONDS, we
    stay quiet so a persistently over-budget dry-run user (or a recurring
    failure) doesn't email on every scheduled run."""
    if mode == "SKIPPED":
        return

    if mode in ("DRYRUN", "ERROR") and prior is not None:
        same_mode = prior.get("mode") == mode
        last_at = int(prior.get("at", 0) or 0)
        if same_mode and (time.time() - last_at) < RENOTIFY_SECONDS:
            logger.info(
                "enforce: suppressing repeat %s notification for user=%s (within re-notify window)",
                mode, user,
            )
            return

    if mode == "REMOVED":
        subject = f"[Bedrock ENFORCE] Removed {user} from Bedrock group(s) ({month})"
        action_line = (
            f"  Action        : REMOVED from group membership(s)\n"
            f"  Groups        : {_fmt_groups(removed)}\n"
        )
        note = (
            "\nThis is reversible: re-add the user to the group(s) above in IAM "
            "Identity Center to restore access.\n"
        )
    elif mode == "DRYRUN":
        subject = f"[Bedrock ENFORCE dry-run] WOULD remove {user} ({month})"
        action_line = (
            f"  Action        : DRY-RUN (no change made)\n"
            f"  Would remove  : {_fmt_groups(dryrun)}\n"
        )
        note = (
            "\nDry-run is ON, so no membership was changed. Set ENFORCE_DRY_RUN "
            "to false to enforce for real.\n"
        )
    else:  # ERROR
        subject = f"[Bedrock ENFORCE error] Failed enforcing {user} ({month})"
        action_line = (
            f"  Action        : ERROR (enforcement failed)\n"
            f"  Failed groups : {_fmt_groups(errored)}\n"
        )
        note = "\nEnforcement will be retried on the next run.\n"

    body = (
        f"Bedrock budget enforcement ran for an over-budget user.\n\n"
        f"  User          : {user}\n"
        f"  User ID       : {user_id}\n"
        f"  Month         : {month}\n"
        f"{action_line}"
    ...
    _notify(subject, body, user, mode)
